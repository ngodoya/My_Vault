> [!INFO]- Rosario
> - **INGENIERÍA DE DATOS**: 11310017
> - **ALGEBRA Y PROGRAMACIÓN LINEAL**: 73210015
> - **MATEMÁTICAS FINANCIERAS**: 15210015
> - **ANÁLISIS ESTADÍSTICO DE DATOS**: 11310013


> [!Question]- Sabana
> - **Álgebra Tensorial y Computacional para Ciencia de Datos** Ciencia de Datos
> - **Bases de Datos** Ciencia de Datos
> - **Laboratorio de Ciencia de Datos: Herramientas, Programación y Proyecto Inicial** Ciencia de Datos
> - **Matemáticas Financieras** Administración de Empresas
> - **Programación y Decisiones** Administración de Empresas

>[!Novia]
>**MODELAMIENTO CLIMÁTICO- AMBIENTAL** 32210011
>**MODELAMIENTO DE ENERGÍA SOLAR** 32710013
>**MODERN RENEWABLE ENERGIES** 32710016

# Proyecto Profesional 2026–2035

## Nombre del Proyecto

Industrial + Estadística + Data Science + Investigación + Docencia

---

# Misión

Construir un perfil interdisciplinario que combine:

- Ingeniería Industrial
- Estadística
- Ciencia de Datos
- Investigación Científica
- Docencia Universitaria

Con el objetivo de:

1. Trabajar internacionalmente.
2. Realizar estudios de posgrado.
3. Convertirse en investigador.
4. Desarrollar una carrera docente.
5. Mantener independencia económica durante el proceso.

---

# Situación Actual

## Formación

Programa principal:

Ingeniería Industrial

Universidad:

Universidad Nacional de Colombia

Sede:

Bogotá

Semestre:

5

PAPA:

4.4

---

## Fortalezas Actuales

### Académicas

- Excelente promedio.
- Buen avance curricular.
- Interés por Estadística.
- Interés por investigación.

### Profesionales

- Iniciativa propia.
- Visión de largo plazo.
- Planeación de carrera.

### Docentes

Actualmente ya se desempeña como:

Tutor personalizado.

Esto implica experiencia temprana en:

- Enseñanza.
- Comunicación.
- Liderazgo académico.
- Resolución de dudas.
- Planeación de sesiones.

Esta experiencia debe considerarse uno de los activos más importantes del perfil.

---

# Visión 2035

## Perfil Profesional Deseado

Graduado de:

- Ingeniería Industrial
- Estadística

Con experiencia en:

- Ciencia de Datos
- Machine Learning
- Optimización
- Analítica Empresarial
- Investigación

Con experiencia internacional:

- Intercambio académico.
- Pasantía internacional.
- Colaboración investigativa.

Con experiencia docente:

- Tutor privado.
- Tutor institucional UNAL.
- Monitor académico.
- Docente universitario.

---

# Los Tres Pilares Estratégicos

## Pilar 1 — Excelencia Académica

Objetivo:

Mantener un rendimiento sobresaliente.

Meta mínima:

PAPA >= 4.3

Meta ideal:

PAPA >= 4.5

Razón:

El promedio abre puertas para:

- Doble programa.
- Intercambios.
- Becas.
- Auxiliarías.
- Posgrados.

---

## Pilar 2 — Perfil Cuantitativo Internacional

Componentes:

### Formación

- Ingeniería Industrial
- Estadística

### Programación

- Python
- SQL
- Git
- Linux

### Ciencia de Datos

- Machine Learning
- Estadística Computacional
- Visualización

### Idiomas

- Inglés C1

### Investigación

- Semilleros
- Grupos de investigación
- Publicaciones

---

## Pilar 3 — Desarrollo Docente

Este pilar tendrá el mismo nivel de importancia que el técnico.

Objetivo:

Convertirse progresivamente en educador universitario.

---

### Etapa 1

Tutor Particular

Estado:

En desarrollo.

Objetivos:

- Mejorar explicación de conceptos.
- Desarrollar metodología propia.
- Construir reputación académica.

---

### Etapa 2

Tutor Institucional UNAL

Objetivo:

Postularse en el próximo semestre.

Beneficios:

- Ingreso económico.
- Experiencia formal.
- Reconocimiento institucional.
- Contacto con profesores.

Meta:

Obtener tutoría institucional antes del semestre 7.

---

### Etapa 3

Monitorías

Objetivo:

Participar como monitor académico.

Prioridad:

Materias cuantitativas.

Ejemplos:

- Cálculo
- Probabilidad
- Estadística
- Investigación de Operaciones

---

### Etapa 4

Mentoría

Objetivo:

Acompañar estudiantes de semestres iniciales.

Desarrollar:

- Liderazgo.
- Orientación académica.
- Gestión de grupos.

---

### Etapa 5

Docencia Universitaria

Objetivo de largo plazo:

Ser profesor universitario.

Idealmente después de:

- Maestría.
- Doctorado.
- Experiencia investigativa.

---

# Ruta por Semestres

## Semestre 6

### Académico

- Mantener PAPA >= 4.4
- Analizar ingreso a Estadística
- Identificar homologaciones

### Docencia

- Consolidar tutorías privadas.
- Documentar experiencias.
- Registrar horas impartidas.

### Programación

Python básico-intermedio.

Proyecto:

Análisis exploratorio de datos.

---

## Semestre 7

### Doble Programa

Solicitar ingreso a Estadística.

### Docencia

Meta principal:

Ingresar a tutorías institucionales.

### Programación

Pandas.
NumPy.
Matplotlib.

### Investigación

Buscar primer grupo.

---

## Semestre 8

### Estadística

Inicio formal del segundo programa.

### SQL

Nivel intermedio.

### Investigación

Participar activamente.

### Docencia

Mantener:

- Tutorías privadas.
- Tutorías institucionales.

---

## Semestre 9

### Ciencia de Datos

Machine Learning inicial.

### Inglés

Preparación TOEFL o IELTS.

### Investigación

Participación en proyectos reales.

---

## Semestre 10

### Movilidad Internacional

Meta prioritaria:

Intercambio académico.

---

### Docencia

Continuar tutorías incluso durante movilidad.

Posibilidad:

Tutorías virtuales.

---

## Semestre 11

### Machine Learning Avanzado

- Árboles
- Random Forest
- Boosting

### Investigación

Buscar coautoría.

---

## Semestre 12

### Especialización

Elegir foco principal:

- Data Science
- IA
- Optimización
- Investigación Operativa

---

## Semestre 13

### Graduación

Finalizar:

- Ingeniería Industrial
- Estadística

---

# Perfil Objetivo al Graduarse

## Formación

✓ Ingeniería Industrial

✓ Estadística

---

## Docencia

✓ Tutor privado

✓ Tutor institucional

✓ Monitor académico

---

## Investigación

✓ Participación en proyectos

✓ Posible publicación

---

## Internacionalización

✓ Intercambio

✓ Contactos internacionales

---

## Programación

✓ Python

✓ SQL

✓ Git

✓ Machine Learning

---

## Idiomas

✓ Inglés C1

---

# Objetivo Final

Convertirse en un profesional capaz de combinar:

- Industria
- Ciencia de Datos
- Investigación
- Docencia

Y tener la posibilidad de elegir entre:

1. Carrera internacional en industria.
2. Maestría y doctorado.
3. Investigación académica.
4. Profesor universitario.
5. Consultoría especializada.
# Diagnóstico y Plan de Actualización de PC

Este documento detalla el estado actual de tu equipo (Intel Core i3-7100) y las mejoras necesarias para adaptarlo a tareas de **Ciencia de Datos** y **Videojuegos ligeros** (era de Xbox 360).

---

## 📋 Estado Actual del Sistema

Según el reporte del comando `lscpu`, tu procesador cuenta con las siguientes características básicas:

*   **Modelo:** Intel(R) Core(TM) i3-7100 CPU @ 3.90GHz (7.ª Generación)
*   **Arquitectura:** x86_64 (Kaby Lake)
*   **Núcleos / Hilos:** 2 núcleos físicos / 4 hilos de procesamiento
*   **Frecuencia Máxima:** 3.90 GHz
*   **Caché L3:** 3 MiB
*   **Sistema Operativo:** Linux (Entorno de terminal Bash)

---

## 🛠️ Plan de Mejoras Recomendadas

Para que tu equipo pueda procesar datos de manera fluida y ejecutar videojuegos sin problemas, es sumamente necesario actualizar los siguientes componentes componentes de hardware:

### 1. Memoria RAM (Prioridad Alta)
La manipulación de grandes volúmenes de datos con librerías de Python (como Pandas) y el uso de navegadores web consumen mucha memoria.
*   **Requisito Mínimo:** 16 GB de RAM en total.
*   **Especificación:** Módulos DDR4 a 2400 MHz o 2666 MHz (el procesador limitará la velocidad a 2400 MHz de forma automática, lo cual es normal).
*   **Marcas sugeridas:** Corsair Vengeance LPX, Kingston Fury o Crucial.

### 2. Tarjeta Gráfica / Video (Esencial)
Los gráficos integrados del procesador son insuficientes. Necesitas una tarjeta dedicada **NVIDIA**, ya que cuenta con núcleos **CUDA**, el estándar absoluto e indispensable para acelerar procesos y modelos en Ciencia de Datos.
*   **Opción Económica:** **NVIDIA GTX 1650 (4GB)**. Ideal para juegos de la época de Xbox 360 a resolución Full HD (1080p) con gráficos en calidad media/alta y para flujos básicos de datos.
*   **Opción Recomendada:** **NVIDIA RTX 3050 (6GB)**. Te ofrece mayor vigencia tecnológica, mejor rendimiento en juegos "chill" modernos y soporte optimizado para Inteligencia Artificial.

### 3. Almacenamiento SSD (Obligatorio)
Si tu sistema operativo Linux y tus archivos se encuentran en un disco mecánico tradicional (HDD), el rendimiento general se verá severamente afectado.
*   **Recomendación:** Instalar un disco de estado sólido (**SSD**) con conexión SATA de al menos 480 GB o 1 TB.
*   **Marcas sugeridas:** Crucial BX500 o Kingston A400.

---

## 📊 Rendimiento Estimado Post-Actualización

Una vez aplicados los cambios, este será el comportamiento esperado de tu computador:

| Tarea                    | Rendimiento Esperado                                                                                                                                                                                          |
| :----------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Ciencia de Datos**     | Excelente para el manejo de bases de datos medianas en Python/R, consultas SQL y aprendizaje local. *Nota: Los modelos masivos de Deep Learning deberán ejecutarse en entornos en la nube como Google Colab.* |
| **Juegos Tipo Xbox 360** | Excelente. Títulos como *GTA V*, *Skyrim*, *Bioshock Infinite* o *Fallout* se ejecutarán de forma fluida a más de 60 FPS estables en resolución 1080p.                                                        |

---

## 🔍 Próximos pasos para validar compatibilidad

Para asegurarnos de que estas piezas se puedan instalar de forma segura en tu gabinete actual, sería de gran ayuda saber:
*   ¿Cuánta **memoria RAM** tienes actualmente instalada en el sistema?
*   ¿De cuántos **Watts** es tu fuente de poder actual? 

# Resumen 2026-1
- Métodos Númericos: (Por definir, entre 4.4-4.7)
- Modelación Matemática: 4.7
- POO: 4.8
- FEM: 3.3
- Explora: 4.7
>[!math]- Caćulo de materias
>```text
>Cálculo en varias variables, 4, 4.4
Ecuaciones Diferenciales, 4, 5.0
Ingeniería estratégica, 3, 4.6
Intensive english I - Internationalization, 4, 4.8
Análisis computacional de datos ambientales, 4, 5.0
Álgebra Lineal, 4, 4.7
Cálculo Integral, 4, 4.7
Fundamentos de mecánica, 4, 3.6
Probabilidad y estadística fundamental, 3, 4.3
Tecnologías cuánticas 100 años revolucionando el mundo., 3, 4.9
Cálculo diferencial, 4, 4.8
Programación de computadores, 3, 4.5
Estadística social fundamental, 3, 4.2
Investigación de mercados, 3, 4.2
Biología general, 3, 3.9
Principios de química, 3, 4.3
Introducción a la ingeniería agrícola, 3, 3.9
Matemáticas Básicas, 4, 4.0
>```


## Posibles Materias 2026-2
### Caso 1
- Introducción a la ingeniería Industrial **(OBLIGATORIA)**, 3 créditos
- Sociología especial: industrial y del trabajo **(OBLIGATORIA)**, 3 créditos
- Probabilidad fundamental **(OBLIGATORIA)**, 4 créditos
- Optimización, 3 créditos
- Explora Portugues, 4 créditos
- Seguridad industrial, 3 créditos
- **Total de créditos:** 20
### Caso 2
- Introducción a la ingeniería Industrial **(OBLIGATORIA)**, 3 créditos
- Sociología especial: industrial y del trabajo **(OBLIGATORIA)**, 3 créditos
- Probabilidad fundamental **(OBLIGATORIA)**, 4 créditos
- Álgebra Matricial, 4 créditos o Fundamentos de matemáticas 4 créditos
- Explora Portugués, 4 créditos
- **Total de Créditos:** 18 créditos
>[!Absctract]- Historia Acádemica
>siabog.unal.edu.co/academia/archivo/historia-academica.do;jsessionid=